...
export const getActivityDetailTool: ToolDef = {
  ...
  handler: async ({ activity_id }: { activity_id: string }, ctx?: ToolContext) => {
    return await intervalsClient.getActivityDetail(activity_id, { signal: ctx?.signal });
  },
};
